...
from function import cancel_appointment, confirm_appointment, get_employees, get_sites, get_product, get_suggestions #, get_employees, get_products, get_product, book_appointment
import logging; 
    ...
    def handle_required_action(self, run_status):
        ...
        for tool_call in tool_calls:
            ...
            if function_name == "get_sites":
                ...
                print(f"get_suggestions called: {arguments.get('city'), arguments.get("service_name")}")
                result =  get_suggestions(arguments.get('city'), arguments.get("service_name"))
...
price_category: understand from user response and choose one from given price categories (`"price_munich_with_card"` or `"price_munich_without_card"` for Munich, and `"price_mittel_with_card"` or `"price_mittel_without_card"`).
Noet: The user will respond for price_category not exactly so you have to figure out which price_category user is talking about by understanding the meaning of response.
